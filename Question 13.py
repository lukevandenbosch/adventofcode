...
data = '''pale chartreuse bags contain 3 faded orange bags.
...
for rule in data.split("\n"):
    outerBagArray = rule.replace(".", "").replace("bags", "bag").split(" contain ")
    outerBag = outerBagArray[0]
    bags[outerBag] = []
    if (outerBagArray[1] != "no other bag"):
        for subBag in outerBagArray[1].split(", "):
            subBagName = subBag.split(" ", 1)[1]
            if subBagName == findBag:
                containerBags += [outerBag]
                total += 1
            bags[outerBag] += [subBagName]
