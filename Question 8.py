...
def validateYear(value, low, high):
    if (value.isnumeric()):
        return (len(value) == 4 and int(value) >= low and int(value) <= high)
    else:
        return False
...
def validateHeight(value):
    if (re.match("^[0-9]+(cm|in)$", value)):
        ...
        if (value[len(value) - 2:len(value)] == "cm"):
...
def validateColour(value):
    return re.match("^#[0-9a-f]{6}$", value)

def validateEye(value):
...
for passport in data.split('\n\n'):
    ...
    for field in validFields:
        if (passport.find(field + ":") == -1):
            ...
            value = re.split(" |\n", passport.split(field + ":")[1])[0]
            if (field == "byr"):
                if (not validateYear(value, 1920, 2002)):
                    failed = True
            elif (field == "iyr"):
                if (not validateYear(value, 2010, 2020)):
                    failed = True
            elif (field == "eyr"):
                if (not validateYear(value, 2020, 2030)):
                    failed = True
            elif (field == "hgt"):
                if (not validateHeight(value)):
                    failed = True
            elif (field == "hcl"):
                if (not validateColour(value)):
                    failed = True
            elif (field == "ecl"):
                if (not validateEye(value)):
                    failed = True
            elif (field == "pid"):
                if (not validatePid(value)):
                    failed = True
    if (not failed):
        count += 1
